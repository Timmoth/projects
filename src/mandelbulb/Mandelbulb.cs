...
{ 
    ...
    public Mandelbulb(GL gl, string vertexShaderSource, string fragmentShaderSource)
    {
        unsafe
        {
            Gl = gl;
    ...
    public void ZoomAtPoint(double px, double py, double factor)
    {
        ...
        _scale *= factor;
        _scale = Math.Max(_scale, 60);
        _scale = Math.Min(_scale, 140);
    ...
    {
        double sensitivity = 0.005; // adjust rotation speed
